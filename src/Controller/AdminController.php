...

{
    ...
    public function PlanningIndex(Request $request,PlanningRepository $planningRepository){
        ...
        if(!$request->getSession()->get("admin"))
        return $this->redirectToRoute('formAdmin');
        $plannings=$planningRepository->findBy([],["classe"=>"ASC"]);
    ...
    public function Planning_eleveIndex(PlanningRepository $planningRepository,Request $request,ClasseRepository $classeRepo,EleveRepository $eleveRepo,TpRepository $tpRepository,NiveauRepository $niveauRepository,Planning_eleveRepository $planning_eleveRepository){
        ...
        foreach($planningRepository->findBy(["classe"=>$request->get("classe")],["date"=>'ASC']) as $planning)
            $option["planning"][]=$planning;

        if(empty($option["planning"]))
            $option["planning"]=$planningRepository->findBy([],["date"=>'ASC']);
        dump($plannings);
        switch($action){
